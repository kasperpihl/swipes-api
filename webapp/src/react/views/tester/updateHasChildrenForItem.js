export default (order, idOrI) => {
  const i = typeof idOrI === 'number' ? idOrI : order.findIndex(item => item.get('id') === idOrI);
  ...
  if(prev) {
    ...
    if(hasChildren !== prev.get('hasChildren')) {
      order = order.setIn([i - 1, 'hasChildren'], hasChildren)
  ...
  if(hasChildren !== curr.get('hasChildren')) {
    order = order.setIn([i, 'hasChildren'], hasChildren)
