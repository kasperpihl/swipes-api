export default (order, idOrI, modifier) => {
  ...
  let i = typeof idOrI === 'number' ? idOrI : order.findIndex(item => item.get('id') === idOrI);
  const newIndent = order.getIn([i, 'indent']) + modifier;

  if(i === 0 || newIndent < 0 || newIndent > order.getIn([i - 1, 'indent']) + 1) {
    return order;
  }
  order = order.setIn([i, 'indent'], newIndent);

  // LEGACY: Indent children to follow it's parent.
  ...
  //     order = order.setIn([i, 'indent'], Math.min(prevIndent + 1, targetIndent));
  //   }
  // }
  return order;
}