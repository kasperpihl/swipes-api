...
export default class Trial extends PureComponent {
  ...
  onExtend() {
    window.open(
      'mailto:help@swipesapp.com?subject=Trial%20extension&body=Dear%20Swipes%20Team,%0D%0A%0D%0ACan%20you%20please%20extend%20my%20free%20trial%20because%20me%20and%20my%20team%20want%20to...'
    );
  }
  onUnpaid = e => {
  ...
  shouldShowPopup() {
    ...
    return (
      !isAccount && (daysLeft <= -7 || (isAdmin && daysLeft < 0 && !dismissed))
    );
  }
  renderTrialIndicator() {
    ...
    return (
      <SW.Indicator expired={expired}>
        <SW.Label onClick={this.onUnpaid}>{text}</SW.Label>
      </SW.Indicator>
    );
  }
  renderTrialPopup() {
      ...
      <SwissProvider displayActions={displayActions}>
        <SW.PopupWrapper>
          <SW.Popup>
            ...
            <SW.Paragraph>
              Subscribe and unite the work of your team in a single place - your
              project goals, files, and communication.
            </SW.Paragraph>
            <SW.Paragraph>
              ⭐&nbsp;&nbsp;Your progress is saved. {actionLbl}
            </SW.Paragraph>
            <SW.Actions>
              {daysLeft > -7 ? (
                <SW.Button title="Dismiss" onClick={this.onDismiss} />
              ) : null}
              {isAdmin ? (
                <SW.Button title="Request extension" onClick={this.onExtend} />
              ) : null}
              {isAdmin ? (
                <SW.Button title="Add billing info" onClick={this.onUnpaid} />
              ) : null}
            </SW.Actions>
          </SW.Popup>
        </SW.PopupWrapper>
