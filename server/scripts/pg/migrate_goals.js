...
const pagination = (resolve, reject) => {
  const tokens = r.table('goals').orderBy('created_at').slice(item, offset);
      ...
      if (results.length > 0) {
        ...
        const rows = results.map((row) => {
          return {
            id: row.id,
            name: row.title,
            plan_id: row.milestone_id,
            organization_id: row.organization_id,
            archived: row.archived,
            assignees: row.assignees,
            attachments: row.attachments,
            attachments_order: row.attachments_order,
        ...
        promises.push(pg.batchInsert('goals', rows));
