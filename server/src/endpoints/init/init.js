...
export default endpointCreate(
  ...
  async (req, res, next) => {
    ...
    const initQ = r
      ...
      .do(user => {
        return user.merge({
          goals: r
            .table('goals')
            .getAll(user('organizations')(0)('id'), {
              index: 'organization_id',
            })
            .filter(goal => {
              return goal('updated_at').during(
                r.ISO8601(timestamp).sub(3600),
                r.now().add(3600)
              );
            })
            .filter(goal => {
              return goal('archived')
                .eq(false)
                .or(goal('archived').eq(!full_fetch));
            })
            .coerceTo('ARRAY'),
        });
      })
      .do(user => {
        return user.merge({
          milestones: r
            ...
            .filter(milestone => {
              return milestone('updated_at').during(
                r.ISO8601(timestamp).sub(3600),
                r.now().add(3600)
              );
            })
    ...
    const servicesQ = r
      .table('services')
      .filter(service => {
        return service('updated_at').during(
          r.ISO8601(timestamp).sub(3600),
          r.now().add(3600)
        );
      })
      .filter(service => {
        return service('hidden')
          .eq(false)
          .or(service('hidden').eq(!full_fetch));
      });

    const notificationsQ = r
      .table('notifications')
      .getAll([user_id, organization_id], { index: 'user_id_organization_id' })
      .filter(notification => {
        return notification('updated_at').during(
          r.ISO8601(timestamp).sub(3600),
          r.now().add(3600)
        );
      })
