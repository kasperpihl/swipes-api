...
const init = (req, res, next) => {
  ...
  if (secret) {
    return updateWebhookReferenceSecret(webhookId, secret)
      .then(() => {
...
const validate = (req, res, next) => {
  ...
  getWebhookReference(webhookId)
    .then((result) => {
      if (result === null) {
        throw new Error('We unsubscribed already. Theese are some old events.');
      }
  ...
  asanaGetAuthDataByAccountId({ accountId })
    .then((accountsAuthData) => {
      accountsAuthData.forEach((accountAuthData) => {
        asana.webhooks(accountAuthData, resourceId, accountId, (err) => {
          if (err) {
