...
import { string, array, object } from 'valjs';
...
import { dbUsersAddOrganization } from './db_utils/users';
import { valLocals, generateSlackLikeId, getDownloadLinks } from '../../utils';
...
// Codes are by country
// WS45366 - Romania
// WS75796 - Germany
// WS96858 - Spain
// WS49858 - Italy
const discount_codes = ['WS45366', 'WS75796', 'WS96858', 'WS49858'];
const organizationsCreate = valLocals(
  'organizationsCreate',
  {
  ...
  (req, res, next, setLocals) => {
    const { user_id, organization_name, discount_code } = res.locals;
    const organizationId = generateSlackLikeId('O');
    let validDiscountCode = null;

    if (discount_codes.includes(discount_code)) {
      validDiscountCode = discount_code;
    }
    ...
    const organization = {
      ...
      trial: {
        started_at: r.now(),
        ending_at: validDiscountCode
          ? r.now().add(3896640)
          : r.now().add(1296000), // 45 days / 14 days
        started_by: user_id,
      },
    };
...
const organizationsAddToUser = valLocals(
...
const organizationsAddPendingUsers = valLocals(
  ...
  (req, res, next, setLocals) => {
    ...
    dbOrganizationsAddPendingUser({ organization_id, user_id: userId })
      .then(result => {
        ...
        const organization =
          organizationChanges.new_val || organizationChanges.old_val;
...
const organizationsGetInfoFromInvitationToken = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { userId, organizationId, invitation_token } = res.locals;
    ...
    return dbOrganizationsGetInfoFromInvitationToken({
      ...
      .then(results => {
        const { me, organization, invited_by } = results;
...
const organizationsGetSingle = valLocals(
...
const organizationsCheckOwnerDisabledUser = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { user_to_disable_id, organization } = res.locals;
    const { owner_id } = organization;

    if (owner_id === user_to_disable_id) {
      return next(
        new SwipesError(
          "The owner can't be disabled. Should transfer owner rights first."
        )
      );
    }
...
const organizationsCheckIsDisableValid = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { user_to_disable_id, organization } = res.locals;
    const { active_users, pending_users } = organization;

    if (
      !active_users.includes(user_to_disable_id) &&
      !pending_users.includes(user_to_disable_id)
    ) {
      return next(
        new SwipesError(
          "This user is not part of that organization or it's disabled from it."
        )
      );
    }
...
const organizationsCheckAdminRights = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { user_id, organization } = res.locals;
    const { owner_id, admins } = organization;
...
const organizationsCheckOwnerRights = valLocals(
...
const organizationsCheckOwnerRightsNot = valLocals(
...
const organizationsPromoteToAdmin = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { organization_id, user_to_promote_id } = res.locals;

    dbOrganizationsPromoteToAdmin({
      organization_id,
      user_id: user_to_promote_id,
    })
      .then(result => {
...
const organizationsDemoteAnAdmin = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { organization_id, user_to_demote_id } = res.locals;

    dbOrganizationsDemoteAnAdmin({
      organization_id,
      user_id: user_to_demote_id,
    })
      .then(result => {
        const changes = result.changes[0];
        const organization = changes.new_val || changes.old_val;
...
const organizationsChangeStripeCustomerEmail = valLocals(
...
const organizationsTransferOwnership = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { user_id, organization_id, user_to_transfer_id } = res.locals;
...
const organizationsDisableUser = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { organization_id, user_to_disable_id } = res.locals;
...
const organizationsDisableAllUsers = valLocals(
...
const organizationsDeletedQueueMessage = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { users_to_notify, organization_id } = res.locals;
...
const organizationsActivateUser = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { organization_id, user_to_activate_id } = res.locals;
...
const organizationsEnableUser = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { organization_id, user_to_enable_id } = res.locals;
...
const organizationsCreatedQueueMessage = valLocals(
...
const organizationsUpdatedQueueMessage = valLocals(
...
const organizationsUserJoinedQueueMessage = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { user_id, organization_id, organization } = res.locals;
...
const organizationsCreateStripeCustomer = valLocals(
  ...
  (req, res, next, setLocals) => {
    ...
    return stripe.customers[funcName](...args)
      .then(customer => {
...
const organizationsCreateSubscriptionCustomer = valLocals(
  ...
  (req, res, next, setLocals) => {
    ...
    const { plan, stripe_customer_id } = organization;
    const subscription = {
      plan:
        plan === 'yearly'
...
const organizationsUpdateSubscriptionQuantity = valLocals(
  ...
  (req, res, next, setLocals) => {
    ...
    const { plan, stripe_customer_id, stripe_subscription_id } = organization;
    const subscription = {
      plan:
        plan === 'yearly'
          ? stripeConfig.yearlyPlanId
...
const organizationsUpdateStripeSubscriptionPlan = valLocals(
  ...
  async (req, res, next, setLocals) => {
    try {
      const { organization_id, plan_to_change, organization } = res.locals;
      ...
      const mappedPlan =
        plan_to_change === 'yearly'
          ? stripeConfig.yearlyPlanId
          : stripeConfig.monthlyPlanId;
      const { plan, stripe_subscription_id } = organization;
...
const organizationsUpdateStripeCardDetails = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { organization, stripe_token } = res.locals;
...
const organizationsCancelSubscription = valLocals(
  ...
  (req, res, next, setLocals) => {
    ...
    const { stripe_subscription_id } = organization;
    const args = [];
...
const organizationsUsersInvitedUserQueueMessage = valLocals(
...
const organizationsMilestoneReorder = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { organization_id, milestone_order } = res.locals;
...
const organizationsMilestoneReorderQueueMessage = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { user_id, organization_id, milestone_order } = res.locals;
...
const organizationsAddMilestone = valLocals(
  ...
  (req, res, next, setLocals) => {
    const { organization_id, milestone_id } = res.locals;
...
const organizationsRemoveMilestone = valLocals(
